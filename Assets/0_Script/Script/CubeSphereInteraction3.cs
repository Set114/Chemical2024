...
public class CubeSphereInteraction3 : MonoBehaviour
{
    ...
    private void UpdateTexts()
    {
        ...
        int count1 = CountN2Spheres(); // 计算N2(sphere)的数量
    ...
    private int CountN2Spheres()
    {
        ...
        foreach (GameObject cube in SphereDetection.cubeIndexMap.Keys)
        {
            ...
            {
                if (sphere.name.StartsWith("N2") && Vector3.Distance(cube.transform.position, sphere.transform.position) < 0.1f)
                {
                    count++;
                }
            }
