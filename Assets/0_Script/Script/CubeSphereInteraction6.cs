...
public class CubeSphereInteraction6 : MonoBehaviour
{
    public TextMeshProUGUI text1; // 第一个TextMeshPro组件
    public TextMeshProUGUI text2; // 第二个TextMeshPro组件
    public TextMeshProUGUI text3; // 第三个TextMeshPro组件
    ...
    private int CountefSpheres()
    {
        ...
        foreach (GameObject cube in SphereDetection.cubeIndexMap.Keys)
        {
            ...
            {
                if (sphere.name.StartsWith("ef") && Vector3.Distance(cube.transform.position, sphere.transform.position) < 0.1f)
                {
                    count++;
                }
            }
    ...
    private int CountO2Spheres()
    {
     ...
     private int CountcSpheres()
    {
        // 计算场景中C(sphere)的数量，限定只计算与Cube相关的球体
        int count = 0;
        ...
        foreach (GameObject cube in SphereDetection.cubeIndexMap.Keys)
        {
            ...
            foreach (GameObject sphere in spheres)
            {
                if (sphere.name.StartsWith("c") && Vector3.Distance(cube.transform.position, sphere.transform.position) < 0.1f)
                {
                    count++;
                }
            }
